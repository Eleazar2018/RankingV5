...
namespace SistemaCadastro
{
    ...
    {        
        ...
        ArrayList ArlListaRanking = new ArrayList();
        ...
        {
            dgvDados.Rows.Clear();
            for (int i = 0; i < ArlListaRanking.Count; i++)
            {
                dgvDados.Rows.Add(((sProduto)ArlListaRanking[i]).Codigo.ToString(), ((sProduto)ArlListaRanking[i]).Nome, ((sProduto)ArlListaRanking[i]).Valor);
            }
        }
        ...
        {
            LimparRegistro();
            txtCodigoRanking.Text = (ArlListaRanking.Count + 1).ToString();
